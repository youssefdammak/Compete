...
import { ArrowLeft, RefreshCw, TrendingUp, TrendingDown, Activity, ChevronDown } from 'lucide-react'
...
export default function CompetitorTrendsPage() {
  ...
  const calculateTrends = useCallback((products: any[]) => {
    if (!products || products.length === 0) return []

    const snapshot: TrendSnapshot = {
      timestamp: new Date().toISOString(),
      avgPrice: products.reduce((sum, p) => sum + (p.price || 0), 0) / products.length,
      discountCount: products.filter((p) => p.price < (p.originalPrice || p.price * 1.2)).length,
  ...
  const refreshTrends = useCallback(async () => {
    ...
    try {
        ...
        if (compResult && compResult.data) {
            const newTrends = calculateTrends(compResult.data.firstTenItems || []);
  ...
  // Prepare chart data
  const priceChartData = trends.length > 0 
    ? trends
        .slice(0, 30)
        .reverse()
        .map((t, idx) => ({
  ...
  const ratingChartData = trends.length > 0
    ? trends
        .slice(0, 30)
        .reverse()
        .map((t, idx) => ({
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      <div className="mb-8">
        ...
        <Button onClick={refreshTrends} disabled={isRefreshing} className="gap-2">
        ...
        <div className="relative">
          ...
          >
            <span className="text-sm font-medium max-w-xs truncate">
              {viewMode === 'seller' ? 'Seller Summary' : `${selectedProduct || 'Select Product'}`}
            </span>
            ...
            <div className="absolute top-full left-0 mt-1 bg-card border border-border rounded-md shadow-lg z-50 w-80 max-h-96 overflow-y-auto">
              <button
                ...
                className={`w-full text-left px-4 py-3 text-sm hover:bg-muted/50 transition-colors border-b border-border ${
                  viewMode === 'seller' ? 'bg-primary/10 text-primary font-semibold' : ''
                }`}
              >
                📊 Seller Summary
              </button>
              {competitor?.firstTenItems && competitor.firstTenItems.map((item, idx) => (
                <button
                  key={idx}
                  onClick={() => {
                    setViewMode('product')
                    setSelectedProduct(item.title || `Product ${idx + 1}`)
                    setShowDropdown(false)
                  }}
                  className={`w-full text-left px-4 py-3 text-sm hover:bg-muted/50 transition-colors border-b border-border last:border-b-0 ${
                    viewMode === 'product' && selectedProduct === item.title ? 'bg-primary/10 text-primary font-semibold' : ''
                  }`}
                  title={item.title || ''}
                >
                  <span className="text-xs text-muted-foreground mr-2">Product {idx + 1}</span>
                  <br />
                  <span className="line-clamp-2">{item.title || `Product ${idx + 1}`}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <Card className="p-4 border-border bg-card">
            <div>
              <p className="text-sm text-muted-foreground mb-1">
                {viewMode === 'seller' ? 'Avg Price Trend' : `${selectedProduct} Price`}
              </p>
              <p className="text-2xl font-bold">
                ${viewMode === 'seller' ? latestTrend.avgPrice.toFixed(2) : (125.99 - Math.random() * 20).toFixed(2)}
              </p>
          ...
          <Card className="p-4 border-border bg-card">
            <div>
              ...
              <p className="text-2xl font-bold">
                {viewMode === 'seller' 
                  ? `${latestTrend.discountCount} / ${competitor.trackedProducts || 10}`
                  : 'In Stock'
                }
              </p>
              <div className="flex items-center gap-1 mt-1">
          ...
          <Card className="p-4 border-border bg-card">
            <div>
              ...
              <p className="text-2xl font-bold">{competitor.avgRating?.toFixed(1) || 'N/A'}</p>
              <div className="flex items-center gap-1 mt-1">
                <Activity className="w-4 h-4 text-blue-500" />
                <span className="text-sm text-blue-500">{competitor.followers?.toLocaleString() || '0'} followers</span>
              </div>
            </div>
          </Card>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card className="p-6 border-border bg-card">
          <h2 className="text-lg font-semibold mb-4">
            {viewMode === 'seller' ? 'Price Trends' : `${selectedProduct} Price Trend`}
          </h2>
          ...
          {priceChartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={priceChartData}>
                ...
                />
                <Line type="monotone" dataKey="price" stroke={viewMode === 'seller' ? '#8b5cf6' : '#3b82f6'} strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
            <p className="text-sm text-muted-foreground py-12 text-center">No price data collected yet</p>
          )}
        </Card>
        ...
        <Card className="p-6 border-border bg-card">
          <h2 className="text-lg font-semibold mb-4">
            {viewMode === 'seller' ? 'Discount Trends' : `${selectedProduct} Availability`}
          </h2>
          <p className="text-sm text-muted-foreground mb-4">
            {viewMode === 'seller' 
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={discountChartData}>
                ...
                {viewMode === 'seller' ? (
                  ...
                  <Bar dataKey="discounted" fill="#10b981" radius={[4, 4, 0, 0]} name="Stock Level" />
                )}
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <p className="text-sm text-muted-foreground py-12 text-center">No data collected yet</p>
          )}
        </Card>
        ...
        <Card className="p-6 border-border bg-card">
          <h2 className="text-lg font-semibold mb-4">
            {viewMode === 'seller' ? 'Seller Rating Trend' : `${selectedProduct} Reviews`}
          </h2>
          <p className="text-sm text-muted-foreground mb-4">
            {viewMode === 'seller' 
          ...
          {ratingChartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={ratingChartData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#333" />
                <XAxis dataKey="date" stroke="#888" style={{ fontSize: '12px' }} />
                <YAxis stroke="#888" style={{ fontSize: '12px' }} domain={[0, 5]} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#1a1a1a', border: '1px solid #333', borderRadius: '8px' }}
                  formatter={(value) => (value as number).toFixed(2)}
                />
                <Line type="monotone" dataKey="rating" stroke={viewMode === 'seller' ? '#10b981' : '#f59e0b'} strokeWidth={2} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
            <p className="text-sm text-muted-foreground py-12 text-center">No rating data collected yet</p>
          )}
        </Card>
        ...
        <Card className="p-6 border-border bg-card">
          <h2 className="text-lg font-semibold mb-4">
            {viewMode === 'seller' ? 'Stock Availability' : `${selectedProduct} Stock`}
          </h2>
          <p className="text-sm text-muted-foreground mb-4">
            {viewMode === 'seller' 
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                ...
                <p className="text-2xl font-bold text-green-500">
                  {viewMode === 'seller' ? stockData[0].inStock : Math.floor(Math.random() * 100 + 50)}
                </p>
              </div>
              <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-3">
                ...
                <p className="text-2xl font-bold text-orange-500">
                  {viewMode === 'seller' ? stockData[0].lowStock : Math.floor(Math.random() * 50)}
                </p>
              </div>
              <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                ...
                <p className="text-2xl font-bold text-red-500">
                  {viewMode === 'seller' ? stockData[0].outOfStock : Math.floor(Math.random() * 500 + 100)}
                </p>
              </div>
            </div>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={viewMode === 'seller' ? stockData : [{ name: selectedProduct, inStock: Math.floor(Math.random() * 100), lowStock: Math.floor(Math.random() * 30), outOfStock: Math.floor(Math.random() * 10) }]}>
                <CartesianGrid strokeDasharray="3 3" stroke="#333" />
      ...
      {viewMode === 'seller' && (
        <>
          <Card className="p-6 border-border bg-card mb-6">
            <h2 className="text-lg font-semibold mb-4">Tracked Products ({competitor.trackedProducts || 0})</h2>
            <div className="space-y-3">
              {competitor.firstTenItems && competitor.firstTenItems.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">No products tracked yet</p>
              )}
            </div>
          </Card>
                ...
                <tbody>
                  {competitor.firstTenItems && competitor.firstTenItems.length > 0 ? (
                    competitor.firstTenItems.slice(0, 10).map((item, idx) => {
                      ...
                      return (
                        <tr key={idx} className="border-b border-border hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => {
                          setViewMode('product')
                          setSelectedProduct(item.title || `Product ${idx + 1}`)
                        }}>
                          <td className="py-3 px-4">
                            <p className="truncate max-w-xs">{item.title || 'Unknown'}</p>
                          </td>
                          <td className="text-right py-3 px-4 font-semibold">${mockPrice.toFixed(2)}</td>
